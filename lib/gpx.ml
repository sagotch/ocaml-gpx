...
and trkseg = { trkpt : wpt list; extensions : extension option }
and copyright = { author : string; year : int option; license : string option }
and link = { href : string; text : string option; typ : string option }
and email = { id : string; domain : string }
and person = { name : string option; email : email option; link : link option }
...
and date_time = float * float option
...
module Of_XML = struct
  ...
  let opt_pcdata xml tag =
    opt_child xml tag
    |> opt_apply (fun x ->
           Xml.children x |> List.map Xml.pcdata |> String.concat "\n")

  let opt_string xml tag = opt_pcdata xml tag
  let opt_float xml tag = opt_pcdata xml tag |> opt_apply float_of_string
  let opt_int xml tag = opt_pcdata xml tag |> opt_apply int_of_string

  let list xml tag =
    try List.find_all (fun x -> Xml.tag x = tag) (Xml.children xml)
    with Not_found -> []

  let time_of_string s : date_time = ISO8601.Permissive.datetime_tz s
  ...
  let email xml = { id = attrib xml "id"; domain = attrib xml "domain" }
...
module To_XML = struct
  ...
  let wrap_int tag int =
    Xml.Element (tag, [], [ Xml.PCData (string_of_int int) ])

  let wrap_float tag float =
    Xml.Element (tag, [], [ Xml.PCData (string_of_float float) ])

  let wrap_string tag string = Xml.Element (tag, [], [ Xml.PCData string ])
  ...
  let wpt (x : wpt) : Xml.xml =
    Xml.Element
      ( "wpt",
        ...
        @@@ opt_apply (fun x -> wrap_string "fix" (string_of_fix x)) x.fix
  ...
  let author (x : person) : Xml.xml =
    Xml.Element
      ( "author",
        [],
        opt_apply (wrap_string "name") x.name
        @@@ opt_apply email x.email @@@ opt_apply link x.link @@@ [] )
  ...
  let rte (x : rte) : Xml.xml =
    Xml.Element
      ( "rte",
        ...
        @@@ opt_apply (wrap_string "type") x.typ
        @@@ x.extensions @@@ List.map rtept x.rtept @ [] )
  ...
  let trk (x : trk) : Xml.xml =
    Xml.Element
      ( "trk",
        ...
        @ opt_apply (wrap_int "number") x.number
        @@@ opt_apply (wrap_string "type") x.typ
        @@@ x.extensions @@@ List.map trkseg x.trkseg @ [] )
  ...
  let metadata (x : metadata) : Xml.xml =
    Xml.Element
      ( "metadata",
        ...
        @@@ opt_apply bounds x.bounds @@@ x.extensions @@@ [] )
